...
- **KEY IDENTIFIER（无符号的1字节）**
  - 密钥标识：与网络接口相结合，识别用于该数据包的 RIPv2 安全关联
...
本节一般性地描述消息处理。 所需的附加算法相关处理将在本文档的单独后续部分中进行描述。 
...
1. RIPv2报文头部的Packet Length字段指示RIPv2报文的整体大小。
2. 根据数据包的出接口，选择适当的 **RIPv2 安全关联**与该数据包一起使用。 可以使用该出站接口的任何有效 RIPv2 安全关联。 适当地填写Authentication Data Offset（认证数据偏移）、Key Identifier（密钥标识符）和Authentication Data Length（认证数据长度）字段。
3. 现在针对“Keyed-MD5”算法或“HMAC-SHA1”算法系列进行算法相关处理。
4. 将得到的认证数据值写入Authentication Data（认证数据）字段。 尾部填充（如果有）实际上并未传输，因为它完全可以根据所使用的消息长度和身份验证算法来预测。
...
2. 根据数据包中`Key Identifier`字段值以及数据包接收的接口，确定相应的RIPv2安全关联（Security Association）。如果在该接口上没有与接收到的`Key Identifier`匹配的有效RIPv2安全关联，那么：
   1. 停止对该数据包的所有处理
   2. RIPv2子系统应记录一个安全事件日志。该日志应至少包含以下信息
      1. 错误数据包接收的日期和时间
      2. 收到的RIPv2数据包的源IP地址
      3. `Key Identifier`字段的值
      4. 接收该错误数据包的接口
      5. 以及在该接口和`Key Identifier`组合中未找到有效的RIPv2安全关联的事实
3. 使用从RIPv2安全关联中指定的算法，根据接收到的RIPv2数据包中的信息和相应的安全关联中的信息，计算出认证数据。
