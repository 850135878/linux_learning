...
- test condition   ` 用来测试文件类型, 或者用来比较字符串`
- [ condition ]    `使用方括号时，要注意在condition两边加上空格。`

```bash
if [ condition1 ]; then   # if [ condition1 ] 等价于 if test condition1
	command1
	command2
...
[[...]]条件判断结构，能够防止脚本中的许多逻辑错误。在[[]]中支持`&&、||、<以及> `操作符。

|       []单括号       |           [[]]双括号           |
| :------------------: | :----------------------------: |
| POSIX 兼容，功能较少 |     Bash 特有，功能更强大      |
|   会进行文件名扩展   | 进行模式匹配，不进行文件名扩展 |
|  使用 `-a` 和 `-o`   |         使用 `&&` 和 `         |
|   不支持正则表达式   |    支持 `=~` 正则表达式匹配    |
|  变量通常需要加引号  |        变量不需要加引号        |
...
> -e:  是否存在   -s：文件非空
>
> -d：是目录      -f：是一般文件        -p：文件是否是管道      -S：这是一个socket
>
> -r：可读          -w：可写         -x：可执行
>
> -L：符号链接  -c：是否字符设备（键盘、modem、声卡等等）  -b：是否块设备（软盘、光驱等）
...
| =                                          | 等于，if [ "$a" = "$b" ]                                     |
| ==                                         | if [ "$a" == "$b" ]                                          |
| !=                                         | if [ "$a" != "$b" ]，这个操作符将在[[ ... ]]结构中使用模式匹配. |
| if [[ "$a" < "$b" ]]或if [ "$a" \\< "$b" ] | 小于, 按照ASCII字符进行排序                                  |
| if [[ "$a" > "$b" ]]或if [ "$a" \\> "$b" ] | 大于, 按照ASCII字符进行排序                                  |
| -z                                         | 字符串为"null"                                               |
| -n                                         | 字符串不为"null"                                             |
...
[[ $a == "z*" ]] # 如果$a与z*相等(就是字面意思完全一样，那么结果将为真
